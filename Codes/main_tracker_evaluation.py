...
Video_Path = "/home/camilo685/Desktop/Tesis/Code/Test_videos/video1L.mp4"
...
#		print (Camera_matrix, focal_length)	
	...
	if matched.size >0:
		for trk_idx, det_idx in matched:
			...
			tmp_trk.det_coord = [z1[0], z1[1], z1[2] - z1[0], z1[3] - z1[1]]
			x_box[trk_idx] = xx
			tmp_trk.box =xx
			tmp_trk.hits += 1
	...
	if len(unmatched_dets)>0:
		if (temp.size)>1:
			...
			tmp_trk.det_coord = [z[0], z[1], z[2] - z[0], z[3] - z[1]]
			tmp_trk.box = xx
			tmp_trk.id = track_id_list.popleft() # assign an ID for the tracker
			tracker_list.append(tmp_trk)
			x_box.append(xx)
...
#			print("Vel= ", vel)
			...
			annotations.append({'dco':True, 'height': detc[2], 'width': detc[3], 'id': trk.id, 'y':  detc[0], 'x': detc[1] })
			
			hip.append({'height': xx[2] - xx[0], 'width': xx[3] - xx[1], 'id': trk.id, 'y':  xx[0], 'x': xx[1] })			
					
	data["frames"].append({'timestamp': frame_count/frameRate, 'num': frame_count, 'class': 'frame', 'annotations':annotations})
	data_hip["frames"].append({'timestamp': frame_count/frameRate, 'num': frame_count, 'class': 'frame', 'hypotheses':hip})
    # Book keeping
...
if __name__ == "__main__":
	...
	data["class"] = 'video'
	data["filename"] = Video_Path
	data_hip["class"] = 'video'
	data_hip["filename"] = Video_Path
	
	complete.append(data)
	complete_hip.append(data_hip)
	
	with open('predictions.json', 'w') as outfile:  
		json.dump(complete_hip, outfile)
